...
socket.on('message', (content, rinfo) => {
    ...
    if(cont3=="1"){
        let sql1 = 'INSERT INTO location SET ?';
        let query = database.query(sql1, cont, (err, result) => {
            if (err) throw err;
        }); 
    }
    else{
        let sql2 = 'INSERT INTO locations SET ?';
...
app.post('/historical', (req, res) => {
        ...
        let sql = `SELECT * FROM location WHERE (lat BETWEEN ${r2numlat} AND ${r1numlat}) AND (lng BETWEEN ${r2numlng} AND ${r1numlng})`;
...
app.post('/create', urlencodedParser, function (req,res) {
    ...
    if (camion2=="on"){
        let sql = `SELECT lat, lng FROM locations WHERE timestamp BETWEEN '${inicio}' and '${fin}'`;
        let query = database.query(sql, (err, result) => {
            if(err){ throw err;}
            //console.log(result);
        io.sockets.emit('historia', result);
